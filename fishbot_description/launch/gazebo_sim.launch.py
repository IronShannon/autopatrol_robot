...
def generate_launch_description():
    ...
    default_world_path = get_package_share_directory("fishbot_description") + "/world/custom_room.world"
    
    # 为 launch 声明参数
    action_declare_arg_mode_path = launch.actions.DeclareLaunchArgument(name="model", default_value=str(default_model_path), description="URDF 的绝对路径")
    ...
    return launch.LaunchDescription([
        ...
        # # 事件动作，当加载 fishbot_joint_state_broadcaster 结束后执行
        # launch.actions.RegisterEventHandler(
        #     event_handler=launch.event_handlers.OnProcessExit(
        ...
        #     )
        # ),
        # 事件动作，当加载 fishbot_effort_controller 结束后执行
        launch.actions.RegisterEventHandler(
            event_handler=launch.event_handlers.OnProcessExit(
                target_action=load_joint_state_broadcaster,
